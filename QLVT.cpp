	...
	while (true)
	{
		while (!_kbhit())
		{
			...
			if (key != 224 && key != 0)
			{
				...
				{
					ThongBao("Khong duoc bo trong !", 5, 5);
					gotoxy(85, 3);
					continue;
				}
			}
...
//					{
//						ThongBao("Vat tu da lap CTHD, KHONG XOA DUOC !", 9, 5);
//						return;
//					}
//				}
//			}
...
void NhapHD(LISTNV &ds, LISTVT tree, char manv[10])
{
	...
	if(strcmp(sohd,"") == 0)
	{
		ThongBao("So hoa don khong duoc bo trong !", 9, 11);
//		NhapHD(ds,tree,manv);
		system("cls");
		return;
	}
	...
	for(int i = 0; i < ds.n; i++)
	{
		for(NODEHD p = ds.dsnv[i]->nodehd; p != NULL; p = p->pNext)
		{
			if(KiemTraSoHD(ds.dsnv[i]->nodehd, sohd) == 1)
			{
				...
				ThongBao("Da co so hoa don nay !", 9, 11);
//				NhapHD(ds,tree,manv);
				system("cls");
				return;
			}
...
void NhapCTHD(LISTNV &ds, LISTVT tree, NODECTHD &dsCTHD, char loai, char sohd[20], char manv[10])
{
	...
	int k = 0;
	for(dsCTHD; dsCTHD != NULL; dsCTHD = dsCTHD->pNext)
	{
		VatTu vt;
		CTHoaDon cthd;
		char mavt[10];
		...
		if(strcmp(dsCTHD->cthd.MaVT,mavt) == 0)
		{
			ThongBao("Cthd da co vat tu nay !", 9, 5);
			system("cls");
			return;
		}
		gotoxy(75, 15 + k);
		...
		do
		{
			...
			{
				for(NODECTHD q = dsCTHD; q != NULL; q = q->pNext)
				{
...
//					system("cls");
				...
				if(cthd.SoLuong < p1->vt.SLT)
				{
					...
					InsertCTHD(dsCTHD, TaoNodeCTHD(cthd));
//					clrscr();
//					system("pause");
...
//nhap cthd
void NhapCTHD_HD(LISTNV ds, LISTVT tree, char loai, char sohd[20], char manv[10])
{
	NhanVien nv;
	while (true)
	{
		...
		if(strcmp(sohd,"") == 0)
		{
			ThongBao("Hay nhap so hoa don de them cthd !", 9, 7);
			NhapCTHD_HD(ds, tree, loai, sohd, manv);
			continue;
		}
				...
				while (flag)
				{
					while (_kbhit())
					{
						...
						case F2:
							...
							InDSHD(listnv.dsnv[vitrinv]->nodehd);
							NhapCTHD_HD(listnv, tree, listnv.dsnv[vitrinv]->nodehd->hd.Loai, listnv.dsnv[vitrinv]->nodehd->hd.SoHD, manv);
							break;
						case F3:
							system("cls");
							gotoxy(9, 3);
